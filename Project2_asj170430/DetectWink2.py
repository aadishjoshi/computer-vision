...
The requirements for the second program are the same as the requirements for the rst program,
except that it must start by applying a lter to the image. The lter can be histogram equalization,
smoothing, or anything else that you may consider to be useful.


commands:
python DetectWink2.py newimages output
python DetectWink2.py
...
def filter(frame, key):
    ...
    if key == 1:
        ...
        gray_frame = cv2.GaussianBlur(dummy_frame, (3, 3), 0)
    elif key == 3:
        gray_frame = cv2.equalizeHist(gray_frame)
...
def detect(frame,key):
    gray_frame = filter(frame, key)
    scaleFactor = 1.1
    minNeighbors = 20
...
def run_on_folder(folder, output):
    ...
    for f in files:
        ...
        if type(img) is np.ndarray:
            ...
            filename = f.split("/")[-1] 
            for option in [0, 1, 2]:
                wink[option],non_detected = detect(img_arr[option], option+1)
